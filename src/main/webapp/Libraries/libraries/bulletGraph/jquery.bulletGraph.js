jQuery.fn.bulletGraph = function(settings) {
    ...
    options.sliderOptions, {
        range: 'min',
        step: 10 / (options.steps.length - 1), //TODO JC 100
        animate: true
    }));
    ...
    function() {
        info.html(select.val() * (1000 / (options.steps.length - 1))); //TODO 100
        actual.slider("option", "value", select.val() * (1000 / (options.steps.length - 1))); //TODO 100
    });
    actual.bind("slidechange", 
    function(event, ui) {
        select.val(ui.value / (1000 / (options.steps.length - 1))); //TODO 100
        info.html(ui.value / (1000 / (options.steps.length - 1))); //TODO 100
    });
    actual.bind("slide", 
	    function(event, ui) {
	        select.val(ui.value / (1000 / (options.steps.length - 1))); //TODO 100
